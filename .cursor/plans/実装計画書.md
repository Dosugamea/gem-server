...
- [ ] ディレクトリ構造の作成（internal/domain, internal/application, internal/infrastructure, internal/presentation）
- [ ] `.gitignore` の設定
- [ ] `README.md` の作成
- [ ] `Makefile` またはビルドスクリプトの作成
...
- [ ] MySQLドライバーの導入
- [ ] Redisクライアントの導入
- [ ] OpenTelemetry関連パッケージの導入
- [ ] JWT認証ライブラリの導入
- [ ] その他必要なパッケージの導入

#### ステップ1.3: 設定管理の実装
- [ ] 設定ファイル構造の定義（環境変数、設定ファイル）
- [ ] 設定読み込み機能の実装（`infrastructure/config/config.go`）
- [ ] 環境別設定ファイルの作成（開発、ステージング、本番）
...
- [ ] `users` テーブルの作成
- [ ] `currency_balances` テーブルの作成
- [ ] `transactions` テーブルの作成
- [ ] `payment_requests` テーブルの作成
- [ ] `redemption_codes` テーブルの作成
- [ ] `code_redemptions` テーブルの作成
- [ ] インデックスの作成
- [ ] 外部キー制約の設定

#### ステップ2.2: マイグレーションスクリプトの作成
- [ ] マイグレーションツールの選定・導入（例: golang-migrate）
- [ ] 初期マイグレーションファイルの作成
- [ ] マイグレーション実行スクリプトの作成
...
#### ステップ3.2: エンティティの実装
- [ ] `domain/currency/currency.go` - Currencyエンティティ
  - [ ] Grant メソッドの実装
  - [ ] Consume メソッドの実装（マイナス残高対応）
  - [ ] バリデーションロジック
- [ ] `domain/transaction/transaction.go` - Transactionエンティティ
- [ ] `domain/payment_request/payment_request.go` - PaymentRequestエンティティ
- [ ] `domain/redemption_code/redemption_code.go` - RedemptionCodeエンティティ
  - [ ] IsValid メソッドの実装
  - [ ] CanBeRedeemed メソッドの実装
  - [ ] Redeem メソッドの実装
...
- [ ] `infrastructure/persistence/mysql/currency_repository.go`
  - [ ] FindByUserIDAndType メソッド
  - [ ] Save メソッド（楽観的ロック対応）
  - [ ] Create メソッド
- [ ] `infrastructure/persistence/mysql/transaction_repository.go`
  - [ ] Save メソッド
  - [ ] FindByTransactionID メソッド
  - [ ] FindByUserID メソッド（ページネーション対応）
- [ ] `infrastructure/persistence/mysql/payment_request_repository.go`
  - [ ] Save メソッド
  - [ ] FindByPaymentRequestID メソッド
  - [ ] Update メソッド
- [ ] `infrastructure/persistence/mysql/redemption_code_repository.go`
  - [ ] FindByCode メソッド
  - [ ] Update メソッド
  - [ ] HasUserRedeemed メソッド
  - [ ] SaveRedemption メソッド

#### ステップ4.2: トランザクション管理の実装
- [ ] `infrastructure/persistence/mysql/transaction_manager.go`
  - [ ] WithTransaction メソッドの実装
...
- [ ] `application/payment/service.go`
...
- [ ] `application/history/service.go`
