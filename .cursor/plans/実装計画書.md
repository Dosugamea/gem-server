...
- [x] `presentation/openapi/spec.yaml` の作成
  - [x] 全エンドポイントの定義
  - [x] スキーマ定義
  - [x] エラーレスポンス定義

#### ステップ6.2: ミドルウェアの実装
- [x] `presentation/rest/middleware/auth.go` - JWT認証ミドルウェア
- [x] `presentation/rest/middleware/tracing.go` - トレーシングミドルウェア
- [x] `presentation/rest/middleware/logging.go` - ログミドルウェア
- [x] `presentation/rest/middleware/error_handler.go` - エラーハンドリングミドルウェア
- [ ] `presentation/rest/middleware/rate_limit.go` - レート制限ミドルウェア（オプション）

#### ステップ6.3: ハンドラーの実装
- [x] `presentation/rest/handler/currency_handler.go`
  - [x] GetBalance ハンドラー
  - [x] GrantCurrency ハンドラー
  - [x] ConsumeCurrency ハンドラー
- [x] `presentation/rest/handler/payment_handler.go`
  - [x] ProcessPayment ハンドラー
- [x] `presentation/rest/handler/code_redemption_handler.go`
  - [x] RedeemCode ハンドラー
- [x] `presentation/rest/handler/history_handler.go`
  - [x] GetTransactionHistory ハンドラー

#### ステップ6.4: ルーティング設定
- [x] `presentation/rest/router.go` の実装
  - [x] ルート定義
  - [x] ミドルウェアの適用
  - [x] OpenAPIバリデーションの統合（Swagger UI / ReDoc統合）

#### ステップ6.5: Swagger UI / ReDoc統合
- [x] Swagger UI の設定
- [x] ReDoc の設定
- [x] OpenAPI仕様ファイルの配信設定
...
- [x] `public/pay/payment-manifest.json` の作成
- [x] HTTPヘッダーでの参照設定（動的生成）

#### ステップ8.2: Web App Manifestの作成
- [x] `public/pay/manifest.json` の作成
- [ ] アイコンの準備（後で追加可能）

#### ステップ8.3: Service Workerの実装
- [x] `public/pay/sw-payment-handler.js` の実装
  - [x] `canmakepayment` イベントハンドラ
  - [x] `paymentrequest` イベントハンドラ
  - [x] 決済アプリウィンドウとの通信
  - [x] PaymentResponseの生成

#### ステップ8.4: 決済アプリウィンドウの実装
- [x] `public/pay/index.html` の作成
  - [x] UIデザイン
  - [x] レスポンシブ対応
- [x] `public/pay/payment-app.js` の実装
  - [x] ユーザー認証処理
  - [x] 残高取得API呼び出し
  - [x] 決済情報の表示
  - [x] 決済承認/キャンセル処理
  - [x] Service Workerとの通信

#### ステップ8.5: 静的ファイル配信の設定
- [x] Echo Frameworkでの静的ファイル配信設定
- [x] 適切なHTTPヘッダーの設定
...
#### ステップ9.1: JWT認証の実装
- [x] JWTトークン生成機能の実装
- [x] JWTトークン検証機能の実装（フェーズ6で実装済み）
- [ ] トークンリフレッシュ機能の実装（オプション）

#### ステップ9.2: 認証ミドルウェアの統合
- [x] REST APIへの認証ミドルウェア適用（フェーズ6で実装済み）
- [x] gRPCへの認証インターセプター適用（フェーズ7で実装済み）
- [x] Payment Handlerへの認証統合（フェーズ8で実装済み）

#### ステップ9.3: 認可ロジックの実装
- [x] ユーザーID検証ロジック（フェーズ6で実装済み）
- [ ] 権限チェックロジック（必要に応じて）
...
- [x] GitHub Actions / GitLab CI などの設定
- [x] 自動テスト実行
- [x] 自動ビルド・デプロイ
...
#### ステップ13.1: APIドキュメント
- [x] OpenAPI仕様の完成
- [x] Swagger UI / ReDoc の動作確認
- [x] API使用例の作成 (`docs/api-examples.md`)

#### ステップ13.2: 開発者向けドキュメント
- [x] アーキテクチャドキュメント (`.cursor/plans/doc-02-アーキテクチャ.md`)
- [x] セットアップガイド (`docs/SETUP.md`)
- [x] 開発ガイドライン (`docs/DEVELOPMENT.md`)

#### ステップ13.3: 運用ドキュメント
- [x] デプロイ手順書 (`docs/DEPLOYMENT.md`)
- [x] 監視・アラート設定ガイド (`docs/MONITORING.md`)
...
7. ✅ フェーズ7: プレゼンテーション層（gRPC API）の実装
8. ✅ フェーズ8: Payment Handlerの実装（基本機能）
9. ✅ フェーズ9: 認証・認可の実装（基本機能）
